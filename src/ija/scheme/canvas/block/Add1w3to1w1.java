...
/**
 * Třída reprezentující blok sčítačky, která má 1 vstupní port s typem "Three Decimal Numbers" a 1 výstupní port s typem "One Decimal Number". Provede sečtení Všech hodnot na vstupním portu a výsledek pošle na výstupní port.
 *
 * @author Radek Hůlka
 * @author Pavel Janko
 */
public class Add1w3to1w1 extends AbstractBlock
{
    private final static int IN_PORT_COUNT = 1;
    private final static String[] IN_PORTS_TYPES = {"Three Decimal Numbers"};
    private final static String[] IN_PORTS_LABELS = {"1.3",};
    ...
    private final static int BLOCK_HEIGHT = 35;
    private final static int BLOCK_WIDTH = 120;
    private final static String BLOCK_NAME = "ADD";

    public Add1w3to1w1(double x, double y, double canvasWidth, double canvasHeight) {
        super.loadFXML(this, "add1w3to1w1.fxml");
    ...
    public void doCalculation() {
        double result = super.getInPort(0).getValue(0) + super.getInPort(0).getValue(1) + super.getInPort(0).getValue(2);
